        ...
        html += `
            <tr>
                ...
                <td class="small">
                    ...
                    <strong class="text-dark d-block">Ent: ${fechaEntTxt}</strong>
                </td>
...
window.entregarPedido = async (id) => {
    ...
    if (!precioActual || precioActual === 0) {
        const { value: nuevoPrecio } = await Swal.fire({
            ...
            input: 'number',
            text: 'Este pedido no tiene precio. Ingresa el precio total acordado:',
            showCancelButton: true,
            inputValidator: (v) => { if (!v || v <= 0) return 'Ingresa un precio válido'; }
        });
    ...
    if (resPago.isConfirmed) {
        ...
        const { value: montoIngresado } = await Swal.fire({
            ...
            showCancelButton: true,
            inputValidator: (v) => { if (!v || v < 0) return 'Ingrese un monto válido'; }
        });
...
// ==========================================
// 6. HISTORIAL DE PEDIDOS Y ABONOS
// ==========================================
function renderizarHistorialPedidos() {
    ...
    tbody.innerHTML = html || '<tr><td colspan="6" class="text-center py-4 text-muted">Aún no hay historial de entregas.</td></tr>';
}
